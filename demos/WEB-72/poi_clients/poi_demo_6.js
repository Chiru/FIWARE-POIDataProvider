...
//        miwi_3d_xhr.open("GET", restQueryURL, true);
    ...
    var log = wex.Util.log, map, geocoder, homeMarker, positionMarker, 
        ...
        BACKEND_ADDRESS_POI = CONFIG.BACKEND_ADDRESS_POI;
...
//        adjust_search_radius(); bounds not yet defined!
...
*/
          ...
          for (category in poi_categories[ontology]) {
            if (category.charAt(0) != '_') {
              ...
              if (poi_categories[ontology][category]._dynamic) {
                by_category[category].dynamic = true;
              }
            }
          }
          by_category._OTHER = {
          ...
          if (by_category.vehicle) {
            by_category.vehicle.selected = true;
            by_category.transportation.expanded = '_ALL';
          }
/*
          by_category._OTHER = {
            label: (poi_categories._OTHER && poi_categories._OTHER._label) 
...
//        locate();
        map.setCenter( new google.maps.LatLng( 60.17043, 
                24.94113 ) );
        searchPOIs();
