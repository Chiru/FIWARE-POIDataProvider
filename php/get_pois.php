<?php // get_pois.php 5.2.1.1 2016-01-27 ariokkon
...
if ($_SERVER['REQUEST_METHOD'] == 'GET' )
{
  ...
  // viewing a POI requires view_permission
  $session = get_session();
  $view_permission = $session['permissions']['view'];

  if (isset ($_GET['poi_id']) && $view_permission)
  {
    $poi_id = $_GET['poi_id'];
    ...
    if (in_array("fw_core", $components))
    {
      ...
      {
        $poi_data = get_fw_core_intl_properties_for_poi($pgcon, 
            $fw_core_intl_tbl, $core_poi_uuid, $fw_core_content);
    ...
    foreach(array_keys($data) as $uuid)
    {
      ...
      foreach($comps_to_update as $comp_name)
      {
        ...
        if($comp_name == "fw_core") {
          ...
          try {
            $collection->update($upd_criteria, $comp_data, 
                array("upsert" => true));
          } catch(Exception $e) {
